...
import { Phone, Mail, Menu, Search, ShoppingBag, X, Heart } from "lucide-react";
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
              ...
              <Link href="/mis-favoritos">
                <button className="hidden xl:flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-slate-50 transition-colors text-slate-700 font-medium relative">
                  <Heart size={20} />
                  <span>Favoritos</span>
                </button>
              </Link>
                  ...
                  <div className="absolute right-0 top-full pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                    <div className="w-64 bg-white rounded-lg shadow-lg border border-slate-200 py-3 flex flex-col">
                      <div className="px-4 pb-3 border-b border-slate-100 mb-2">
                        <p className="text-sm font-semibold text-slate-900 mb-2">Bienvenido a {siteConfig.name}</p>
                        <Link href="/auth/login">
              ...
              <Link href="/mis-favoritos" className="xl:hidden p-2 hover:bg-slate-50 rounded-full text-slate-600">
                <Heart size={21} />
              </Link>
              <Link href="/carrito" className="xl:hidden p-2 hover:bg-slate-50 rounded-full text-slate-600 relative">
                <ShoppingBag size={21} />
                {itemCount > 0 && (
                        ...
                        <div key={idx} className="mb-3 last:mb-0">
                          ...
                          {col.items && (
                            <ul className="pl-2 border-l-2 border-slate-200 space-y-1">
                              {col.items.map((item: { label: string; href: string }, i: number) => (
                                <li key={i}>
                                  <Link 
                                    href={item.href} 
                                    onClick={() => setIsMobileMenuOpen(false)}
